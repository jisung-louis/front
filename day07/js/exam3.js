...
let avg = sum2 / scores.length;

console.log("합계 : ", sum2, "평균 : ", avg);
...
for(let i = 0; i < scores2.length; i++){
    if(scores2[i] >= 100){
        console.log("100점 만점자를 찾았습니다! 인덱스 : ", i);
        break;
    }
}
...
for(let i = 0; i < nameArray.length; i++){
    ...
    for(let j = 0; j < scores4[i]/10; j++){
        output += `●`;
    }
    for(let j = 0; j < 10 - scores4[i]/10; j++){
        output += `○`;
    }
    output += `</div>`;
}
